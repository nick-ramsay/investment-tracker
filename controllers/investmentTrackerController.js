...
module.exports = {
    ...
    generateInvestmentData: function (req, res) {
        ...
        axios.get("https://cloud.iexapis.com/stable/stock/" + investmentData.symbol + "/quote?token=" + keys.iex_credentials.apiKey).then(function (res) {
            db.Portfolios
                .updateOne({ _id: portfolioID, account_id: accountID, "investments.symbol": res.data.symbol },
                    {
                        $set: { "investments.$.name": res.data.companyName, "investments.$.price": res.data.latestPrice, "investments.$.peRatio": res.data.peRatio, "investments.$.target_percentage": Number(investmentData.target_price / res.data.latestPrice).toFixed(2) }
                    }
                )
